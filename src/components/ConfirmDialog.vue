...
const props = withDefaults(defineProps<{ title: string; message: string; confirmLabel?: string; danger?: boolean; busy?: boolean }>(), { confirmLabel: 'Confirm', danger: false, busy: false })
const emit = defineEmits<{ confirm: []; cancel: [] }>(), panel = ref<HTMLElement>()
let previous: Element | null
function key(e: KeyboardEvent): void {
  if (e.key === 'Escape') { e.preventDefault(); if (!props.busy) emit('cancel') }
  if (e.key !== 'Tab') return
...
onBeforeUnmount(() => { document.removeEventListener('keydown', key); if (previous instanceof HTMLElement && previous.isConnected) previous.focus() })
</script>
